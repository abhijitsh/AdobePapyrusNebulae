...
// Function to Obtain all Output Headers from parsed JSONs
const {extractBillInfo} = require('./InvoiceFormat.js');

// SET UP SDK
const credentials = API_SDK.Credentials
    .serviceAccountCredentialsBuilder()
    .fromFile('./pdfservices-api-credentials.json')
    .build();

    // Create client config instance with custom time-outs.
    const clientConfig = API_SDK.ClientConfig
    .clientConfigBuilder()
...
const options = new API_SDK.ExtractPDF.options.ExtractPdfOptions.Builder()
    .addElementsToExtract(API_SDK.ExtractPDF.options.ExtractElementType.TEXT).build();
...
function extractPDF(Input_Pdf, Output_Zip, Num, timeout) {
    ...
    extractPDFOperation.execute(Context)

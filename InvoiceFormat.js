// Function to add an Bounds of Records in CSV File
const addRecord = require('./csvcreate');

module.exports.extractBillInfo = async function extractBillInfo(extractedInfo){

    // Traverse All the Parsed Objects.
    for( let i = 0; i < extractedInfo.length; i++){

        let data = extractedInfo[i];        // Current Invoice Object.
        let invoice = {};                   // To Store the Invoice headings for all items.
        ...
        // Traversing and Grouping every element in respective category.
        ...
        invoice.customerLine1 = customerDetails[curr++] + " " + customerDetails[curr++] + " " + customerDetails[curr++];
    ...
    invoice.businessName = data[position++].Text.trim();
        ...
        while(data[position].Text==undefined || !data[position].Text.startsWith('AMOUNT')){
            position++;
        }
        position++;
